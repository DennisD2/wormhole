...
public class DecoratorTest {
	...
	public void testI2CRead() {
		//loadImages("/home/dennis/tek1241_i2c_5us_chained_2_address_a0.tek");
		//loadImages("/home/dennis/tek1241_i2c_5us_chained_1.tek");
		loadImages("/home/dennis/tek1241_i2c_5us_chained_good1.tek");
	}
	...
	public void loadImages(String file) {
		...
		int triggerPos = tek.getTekRaw().getRawTpi(
				tek.getTekRaw().C_TB_TIMEBASE1);
		System.out.println("Timebase 1 clocks after trigger event: " + n);

		int clock[] = tek.getTimelineBits(0, 0);
		int data[] = tek.getTimelineBits(0, 1);
		for (i = 0; i < clock.length; i++)
			System.out
					.println(i + " clock: " + clock[i] + ", data: " + data[i]);
		
		I2CDecorator decorator = new I2CDecorator();
		decorator.init(tek);
		decorator.decorate();
		decorator.print();
